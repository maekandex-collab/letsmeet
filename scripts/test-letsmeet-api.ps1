...
$notifs = Invoke-LetsMeet GET "/api/letsmeet/notification/list?page=1" -Token $token
if ($notifs.status -eq 200) {
  $notifJson = $notifs.body | ConvertFrom-Json
  if ($null -ne $notifJson.items) {
    Write-Result "GET /api/letsmeet/notification/list" "PASS" "count=$($notifJson.count) items=$($notifJson.items.Count)"
  } else {
    Write-Result "GET /api/letsmeet/notification/list" "WARN" "200 but missing items array: $($notifs.body)"
  }
} else {
  Write-Result "GET /api/letsmeet/notification/list" "FAIL" "$($notifs.status) $($notifs.body)"
}
